...
export default class D_add extends Component{
    constructor(){
        super()
        this.state={
            item:{},
    ...
    componentDidMount(){
       return axios.get(`/api/product_d/${this.props.match.params.id}`).then(response=>{
            this.setState({item:response.data[0]})
        })
    }

    updateBin(name,price,img){
        axios.post(`/api/product_d/${this.props.match.params.id}`,{product_name:name,price:price,img:img})
    ...
    render(){
        return(
            <div>
               ...
               <Link to='/delta' className="add-title"> <h1 >Shelf D</h1></Link>
                ...
                <br/>
                <Link to ='/delta'><button className="add-button" onClick={()=>this.updateBin(this.state.name,this.state.price,this.state.img)}><h1 className="add-button-text">+ Add Inventory</h1></button></Link>
