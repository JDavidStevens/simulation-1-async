...
export default class InventoryB extends Component{
    constructor(){
        super()
        this.state={
            item:{},
    ...
    componentDidMount(){
       return axios.get(`/api/product_b/${this.props.match.params.id}`).then(results=>{
    ...
    deleteProduct(id){
        axios.delete(`/api/product_b/${id}`).then(results => 
        this.setState({item:results.data[0]}))
    }
    ...
    render(){
        return(
            ...
            <div>
               ...
               <Link to = '/bravo' className="inventory-title"> <h1 className="inventory-title">Shelf B</h1></Link>
    ...
    alt="pic"/>
               ...
               { (this.state.disabled)?(
                    <div>
                        ...
                        <Link to='/bravo'><button className='delete-button' onClick={()=>this.deleteProduct(this.props.match.params.id)}>DELETE</button></Link>
                        </div>
                    </div>
                    ):(
                    <div>
