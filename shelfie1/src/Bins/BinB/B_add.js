...
export default class B_add extends Component{
    ...
    updateBin(name,price,img){
        axios.post(`/api/product_b/${this.props.match.params.id}`,{product_name:name,price:price,img:img})
        ...
        return(
            <div>
               ...
               <Link to='/bravo' className="add-title"> <h1 >Shelf B</h1></Link>
                ...
                <br/>
                <Link to ='/bravo'><button className="add-button" onClick={()=>this.updateBin(this.state.name,this.state.price,this.state.img)}><h1 className="add-button-text">+ Add Inventory</h1></button></Link>
